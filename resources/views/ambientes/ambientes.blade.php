                        ...
                        @foreach ($mostrar as $item)
                            <tr>
                                {{--<td>
									...
									<td>{{$item->nro_elementos}}</td>
									<td>{{$item->descripcion}}</td>
									<td>{{$item->codigo_centro}}</td>
								<td>
                                    <a href="{{route('ambientes.editar',"id=".$item->nro_ambiente)}}" class="edit" data-toggle="modal"><i
                                            class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
                                    <a href="{{route('ambientes.eliminar',$item->nro_ambiente)}}" class="delete" data-toggle="modal"><i
    ...
    <div id="addEmployeeModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="{{route('ambientes.crear')}}">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="nro_ambiente" required>
                        </div>
						...
						<div class="form-group">
                            ...
                            <input type="number" class="form-control" name="nro_elementos" required>
                        </div>
                        ...
                        <select  class="form-group" name="codigo_centro">
                            @foreach ($sedes as $item)
                                <option value="{{ $item->codigo_centro}}">{{ $item->nombre}}</option>
                            @endforeach
                        </select>
                    </div>
